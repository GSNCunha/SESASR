...
def imu_sm_simpy():
    ...
    # Update hx to account for all variables (x, y, theta, v, w, mx, my)
    eval_hx_imu = sympy.lambdify((w), hx, "numpy")
    
    # Compute the Jacobian of hx with respect to [x, y, theta, v, w]
    Ht = hx.jacobian(Matrix([w]))  # Jacobian w.r.t. the full state vector
    eval_Ht_imu = squeeze_sympy_out(sympy.lambdify((w), Ht, "numpy"))

    return eval_hx_imu, eval_Ht_imu
...
def odom_sm_simpy():
    ...
    # Update hx to account for all variables (x, y, theta, v, w, mx, my)
    eval_hx_odom = squeeze_sympy_out(sympy.lambdify((v,w), hx, "numpy"))
    
    # Compute the Jacobian of hx with respect to [x, y, theta, v, w]
    Ht = hx.jacobian(Matrix([v,w]))  # Jacobian w.r.t. the full state vector
    eval_Ht_odom = squeeze_sympy_out(sympy.lambdify((v,w), Ht, "numpy"))

    return eval_hx_odom, eval_Ht_odom
