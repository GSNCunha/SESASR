...
class RobotEKF:
    ...
    def updateIMU(self, z, eval_hx, eval_Ht, Qt, Ht_args=(), hx_args=(), residual=np.subtract):
        """Performs the update innovation of the extended Kalman filter.
        ...
        # Compute the Kalman gain, you need to evaluate the Jacobian Ht
        Ht = eval_Ht(*Ht_args)
        SigmaHT = self.Sigma[4,4]*Ht
        self.SImu = Ht*SigmaHT + Qt 
        self.KImu = SigmaHT/self.SImu
        
        """
        print('Ht,SigmaHT,SImu,KImu:')
        print(Ht)
        print(SigmaHT)
        print(self.SImu)
        print(self.KImu)
        ...
        self.mu[4] += self.KImu*y

        # P = (I-KH)P(I-KH)' + KRK' is more numerically stable and works for non-optimal K vs the equation
        # P = (I-KH)P usually seen in the literature.
        # Note that I is the identity matrix.
        I_KH = 1 - self.KImu*Ht
        self.Sigma[4,4] = self.Sigma[4,4]*I_KH + self.KImu*Qt*self.KImu

    def updateOdom(self, z, eval_hx, eval_Ht, Qt, Ht_args=(), hx_args=(), residual=np.subtract):
        """Performs the update innovation of the extended Kalman filter.
        ...
        if np.isscalar(z):
            z = np.asarray([z], float)
        # Compute the Kalman gain, you need to evaluate the Jacobian Ht
        Ht = np.diag(eval_Ht(*Ht_args))
        SigmaHT = self.Sigma[3:5,3:5] @ Ht.T
        self.SOdom = Ht @ SigmaHT + Qt 
        self.KOdom = SigmaHT @ inv(self.SOdom)
        print(Ht)
        """
        print('Ht,SigmaHT,SImu,KImu:')
        ...
        self.mu[3:5] += self.KOdom @ y

        # P = (I-KH)P(I-KH)' + KRK' is more numerically stable and works for non-optimal K vs the equation
        # P = (I-KH)P usually seen in the literature.
        # Note that I is the identity matrix.
        I_KH = 1 - self.KOdom*Ht
        self.Sigma[3:5,3:5] = self.Sigma[3:5,3:5] @ I_KH + self.KOdom @ Qt @ self.KOdom
