...
from EKF2_package.ekf2 import *
from EKF2_package.probabilistic_models2 import *
import yaml
...
class Ekf(Node):
    def __init__(self):
        ...
        self.qt_imu = 0.1 # Imu noise covariance
        self.qt_odom = np.eye(2)*0.1 # Odom noise covariance
    ...
    def load_landmarks(self):
        with open('/home/pablo/ros2_ws/install/turtlebot3_perception/share/turtlebot3_perception/config/landmarks.yaml', 'r') as file:
    ...
    def odom_callback(self, msg: Twist):
        # Not being used because /odom doesn't work
        odom_msg = Odometry()

        self.v = msg.linear.x
        self.w = msg.angular.z

        measurement = [self.v, self.w]

        eval_hx_odom,eval_Ht_odom = odom_sm_simpy()
        self.ekf.updateOdom(measurement,
                        eval_hx_odom,eval_Ht_odom,
                        self.qt_odom,
                        (self.ekf.mu[3],self.ekf.mu[4]),
                        (self.ekf.mu[3],self.ekf.mu[4]),
    ...
    def imu_callback(self,msg: Imu):
        ...
        eval_hx_imu,eval_Ht_imu = imu_sm_simpy()
        self.ekf.updateIMU(msg.angular_velocity.z,
                        eval_hx_imu,eval_Ht_imu,
                        self.qt_imu,
                        ([self.ekf.mu[4]]),
                        ([self.ekf.mu[4]]),
                        residual = np.subtract)
